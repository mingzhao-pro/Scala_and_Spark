...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'rankLangs' should work for RDD with two elements") {
    ...
    val rdd = sc.parallelize(List(WikipediaArticle("1", "Scala is great JavaScript"), WikipediaArticle("2", "Java is OK, but Scala is cooler JavaScript")))
  ...
  test("'rankLangsUsingIndex' should work for a simple RDD with three elements") {
    ...
    val articles = List(
        WikipediaArticle("1","Groovy is pretty interesting, and so is Erlang"),
        WikipediaArticle("2","Scala and Java run on the JVM JavaScript JavaScript"),
        WikipediaArticle("3","Scala is not purely functional")
      )
    val rdd = sc.parallelize(articles)
    val index = makeIndex(langs, rdd)
